 ...
 * @param {{ region?: 'US'|'CA', currency?: string }} [opts]
 *
 * NOTE: Copart uses Angular SPA and aggressive bot-detection (Cloudflare).
 */
async function scrapeCopart(url, opts = {}) {
  const { region = 'US', currency = 'USD' } = opts;
  ...
  try {
    const context = await browser.newContext({
      ...
      locale: isCa ? 'en-CA' : 'en-US',
      extraHTTPHeaders: { 'Accept-Language': isCa ? 'en-CA,en;q=0.9' : 'en-US,en;q=0.9' },
    });
    ...
    if (/incapsula|_Incapsula_Resource/i.test(rawHtml || '')) {
      throw new Error('Strona blokuje automatyczny dostęp (Incapsula) — użyj PDF fallback');
    }
    ...
    // ── Extract data ──────────────────────────────────────────────
    const data = await page.evaluate((isCA) => {
      ...
      const odoKmM = body.match(/Odometer[:\s]*([\d,]+)\s*km/i)
        || (isCA ? body.match(/([\d,]+)\s*km/i) : null);
      const odoMiM = body.match(/Odometer[:\s]*([\d,]+)\s*mi/i)
        || (isCA ? null : body.match(/Mileage[:\s]*([\d,]+)/i));
      const odometerKm = odoKmM ? parseInt(odoKmM[1].replace(/,/g, ''), 10) : null;
      ...
      return {
        ...
        primaryDamage, secondaryDamage, odometer, odometerKm, odometerMi, runDrive,
        saleDate, location, estimatedValue,
        images: [...new Set(images)].slice(0, 15),
      };
    }, isCa);

    return { source: 'Copart', region, currency, data };
